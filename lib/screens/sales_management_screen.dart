...
class _SalesManagementScreenState extends State<SalesManagementScreen> with SingleTickerProviderStateMixin {
          ...
          itemBuilder: (context, index) {
            ...
            final driverData = driver.data() as Map<String, dynamic>;
                  ...
                  children: [
                    ...
                    SizedBox(height: 8),
                    ElevatedButton.icon(
                      onPressed: () => _generateDriverSalesReport(driver.id, driverData),
  ...
  Widget _buildSalesSummaryCards() {
    ...
    final averageOrder = totalTransactions > 0 ? totalSales / totalTransactions : 0.0;
  ...
  Widget _buildSalesListTile(Map<String, dynamic> data) {
    ...
    return ListTile(
      leading: CircleAvatar(
        backgroundColor: paymentStatus == 'paid' ? Colors.green.withOpacity(0.2) : Colors.orange.withOpacity(0.2),
        child: Icon(
      ...
      subtitle: Text('手数料: ¥${commission.toStringAsFixed(0)} | 純利益: ¥${netAmount.toStringAsFixed(0)}'),
      trailing: Column(
        mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget _buildDriverSalesCard(String driverId, Map<String, dynamic> driverData) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            StreamBuilder<QuerySnapshot>(
              stream: _firestoreService.getDriverSales(driverId, _selectedMonth),
              builder: (context, snapshot) {
                if (!snapshot.hasData) {
                  return Row(
                    children: [
                ...
                return Row(
                  children: [
                    Expanded(child: _buildDriverStatItem('売上', '¥${totalSales.toStringAsFixed(0)}')),
                    Expanded(child: _buildDriverStatItem('件数', '${totalCount}件')),
                    Expanded(child: _buildDriverStatItem('平均', '¥${average.toStringAsFixed(0)}')),
                  ],
                );
              },
            ),
  ...
  Future<void> _generateMonthlySalesInvoice() async {
    try {
      ...
      if (salesSnapshot.docs.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${_getMonthName(_selectedMonth)}の売上データがありません')),
        );
        return;
      }
  ...
  // ドライバー個人売上明細書生成
  Future<void> _generateDriverSalesReport(String driverId, Map<String, dynamic> driverInfo) async {
    try {
      // ドライバーの売上データ取得
      final salesSnapshot = await FirebaseFirestore.instance
          .collection('sales')
